...
stds_fig2way <- stds[stds$Host != "Pistachio" & stds$Host != "Walnut", ]
...
unique(stds_fig2way$Host)
# [1] "Almond" "Fig" 

#-- 1. Perform and annotate PCA ---------------------------------------------

pca <- prcomp(stds_fig2way[,-1], # drops Species, all numeric data frame
              center = TRUE,
              scale. = TRUE) 
...
plda <- predict(object = lda,
                newdata = stds_fig2way) # plda is a list of three objects



#-- 3. Graphical comparisons of PCA and LDA -----------------------------------

dataset = data.frame(Host = stds_fig2way[,"Host"], # take single col frm src dat
